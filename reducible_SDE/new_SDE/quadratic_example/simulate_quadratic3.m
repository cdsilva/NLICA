function [data_init, data_burst_init, data1, data1_burst, t] = simulate_quadratic3(nsteps, dt, nsteps_burst, dt_burst, epsilon, data_init)
...
quad_scale = 0.2;
sin_scale = 1;
f1 = @(x) [x(:,1)+quad_scale*x(:,1).*x(:,2).^2/epsilon sin_scale*sin(x(:,1)*pi)+x(:,2)/sqrt(epsilon)];
