...
\footnotetext[3]{Department of Electrical Engineering, Technion – Israel Institute of Technology, Haifa, Israel, 3200003}
...
%
However, in general, this reduction introduces memory terms \cite{roberts2008normal}.
%
...
in this case, it is possible (for weak models or for models with linear noise) to obtain a reduced system of differential equations in only the slow variables {\em without memory terms} \cite{roberts2008normal}.
%
...
Given a system of ordinary differential equations with an explicit functional form,
one can make numerical approximations, such as the quasi-steady state approximation \cite{segel1989quasi} or
the partial equilibrium approximation \cite{gallagher1986combined}, to reduce the system dimensionality.
%
...
We present detailed analysis for our method, and provide conditions under which it will successfully recover the slow variables.
%
Furthermore, based on this analysis, we present data-driven protocols to tune the parameters of the method appropriately.
%
Our presentation and discussion addresses two-time-scale stochastic systems; however, we claim that our framework and analysis readily extends to systems with multiple time scale separations.
%
...
\includegraphics[width=\textwidth]{data_init_nonlinear}
\caption{}
\end{subfigure}
%
\begin{subfigure}{0.3\textwidth}
...
%
\begin{subfigure}{0.3\textwidth}
\includegraphics[width=\textwidth]{data_rescaled}
\caption{}
\end{subfigure}
\caption{}
\label{fig:data_rescaling}
\end{figure}

In this work, we assume that we observe data $\mathbf{y}(t_1), \dots, \mathbf{y}(t_N)$, where $t_i = t_0 + i \Delta t$, collected from a single trajectory of a stochastic process $\mathbf{y}(t)$. 
%
We further assume that there exists a nonlinear coordinate transform $\mathbf{f}$ such that $\mathbf{y}(t) = \mathbf{f} \left( \mathbf{x} (t) \right)$, where $\mathbf{x}$ comes from the following autonomous two-time-scale system of stochastic differential equations (SDEs),
...
%
Here, we restrict ourselves to the case where $\mathbf{x}(t)$ comes from an SDE with additive noise; the case of multiplicative noise is the subject of future research.
%
The parameter $\epsilon$ induces a separation of time scales, such that $\mathcal{O}(1)$ changes in $x_1, \dots, x_m$ occur over times scales $\mathcal{O}(1)$, and $\mathcal{O}(1)$ changes in $x_{m+1}, \dots, x_n$ occur over time scales $\mathcal{O}(\epsilon)$.

Our goal is extract a parameterization of the observed data $\mathbf{y}(t)$ which is consistent with the underlying {\em slow} variables $x_1, \dots, x_m$. 
%
We opt to use a data-driven technique (diffusion maps) which does not directly utilize any information about the underlying dynamics.
%
The main component in the diffusion maps algorithm is defining the relevant pairwise distance metric.
%
Therefore, to analyze dynamic data using diffusion maps, we first need to transform our data to (1) factor out the effects of the (perhaps nonlinear) function $\mathbf{f}$, and (2) rescale the data so that the length scale fluctuations are commensurate with the time scale fluctuations (see Figure~\ref{fig:data_rescaling}). 
%
In this work, we propose a distance metric which incorporates these two transformations in a single step. 

Such a rescaling transformation of the SDEs in \eqref{eq:general_SDE} is guaranteed to exist for Khasminskii systems; however, we suspect that our method will be applicable for a broader class of multiscale SDEs. 
%
We are interested in the more general case where we observe $\mathbf{y}(t) = \mathbf{f}( \mathbf{x}(t))$. 
%
In order to factor out the effects of $\mathbf{f}$, we assume that $\mathbf{g} = \mathbf{f} ^{-1}$ is well-defined on the image of $\mathbf{f}$, and both $\mathbf{f}$ and $\mathbf{g}$ be continuously differentiable to fourth order.
%
For the examples presented here, we have $\mathbf{f}: \mathbb{R}^d \mapsto \mathbb{R}^d$; however, we note that in general, the function $\mathbf{f}: \mathbb{R}^n \mapsto \mathbb{R}^d$, $n \le d$, can be a deterministic (possibly nonlinear) function whose image is an $n$-dimensional manifold $\mathcal{M}$ in $\mathbb{R}^d$.
%
We further note that the data must be sampled such that $N \Delta t \gg \epsilon$; this condition ensures that fast variables have equilbrated, and the slow variables have evolved appreciably. 
...
%
%Because of the separation of time scales in the diffusion terms, a short burst of simulation (with $\delta t = o \left( \epsilon \right)$) will yield a cloud of points which is broadly distributed ($\mathcal{O} \left( 1/\sqrt{\epsilon} \right)$) in the last $n-m$ fast directions but narrowly distributed ($\mathcal{O} \left( 1 \right)$) in the first $m$ slow ones.
%
...
%Assuming the ensemble average of $a_i(\mathbf{x})$, conditioned on the slow variables $x_1, \dots, x_m$, converges to a distribution which is {\em only} a function of the slow variables, then by the averaging principle \cite{freidlin2012random}, we can write a reduced SDE in {\em only} the slow variables $x_1, \dots, x_m$.
%
%We assume that our data is collected over a time scale much larger than $\epsilon$, such that the $n-m$ fast variables have saturated and we have observed appreciable evolution in the slow variables. 
%
...
%
%In general, we are not given the variables $\mathbf{x}(t)$ from the original SDE system, but instead, we are given some {\em observations} in the form $\mathbf{y}(t) = \mathbf{f} (\mathbf{x}(t))$.
%
...
\end{aligned}
\end{equation}
and a short burst of simulation (with $\delta t = o \left( \epsilon \right)$) yields a cloud of points which is broadly distributed ($\mathcal{O} \left( 1/\sqrt{\epsilon} \right)$) in the last $n-m$ fast directions but narrowly distributed ($\mathcal{O} \left( 1 \right)$) in the first $m$ slow ones.
%
The Mahalanobis distance between samples is
\begin{equation} \label{eq:rescale_x_dist}
\| \mathbf{x}(t_2) - \mathbf{x}(t_1) \|^2_M = \sum_{i=1}^n e_i \left( x_i(t_2) - x_i(t_1) \right)^2.
\end{equation}
Note that the metric in \eqref{eq:rescale_x_dist} is insensitive to variations in the fast variables.
%
...
\end{equation}
%
Clearly, this approximation is accurate when $\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|_2^4 \ll \| \mathbf{z}(t_2) - \mathbf{z}(t_1) \|^2_2$; in Section~\ref{subsec:obsfn}, we provide a more detailed analysis of the errors incurred in this approximation.
%
Because we integrate these distances into a manifold learning algorithm which only considers local distances, we recover a parametrization of the data which is consistent with the underlying system variables $\mathbf{x}(t)$, even when the data are obscured by a function $\mathbf{f}$.
%
Section~\ref{subsec:cov_est} shows how we approximate this distance directly from data $\mathbf{y}(t)$.
%
...
Following \cite{singer2008non}, we show via Taylor expansion that the Mahalanobis distance between the observations \eqref{eq:mahalanobis_distance} approximates the Euclidean distance {\em in the rescaled variables} for general nonlinear observation functions $\mathbf{f}$ (provided $\mathbf{f}$ is bilipschitz and both $\mathbf{f}$ and $\mathbf{f}^{-1}$ are differentiable to fourth order).
...
%
We estimate the covariance $\mathbf{C}(\mathbf{y}(t_0))$ empirically from a set of values $\mathbf{y}(t_1), \dots, \mathbf{y}(t_q)$ drawn from the local distribution at $\mathbf{y}(t_0)$, by running $q$ simulations for a short time $\delta t = o (\epsilon) $, each starting from $\mathbf{y}(t_0)$; we refer to this as a {\em simulation burst}.
%
Alternatively, we can consider a single time series of length $q \delta t = o (\epsilon) $ starting from $\mathbf{y}(t_0)$, and then estimate the covariance
...
%
One source of error, discussed in Section~\ref{subsec:obsfn}, is approximating the function $\mathbf{f}$ locally as a linear function by truncating the Taylor expansion of $\mathbf{f}$ at first order.
%
An additional source of error, discussed in Section~\ref{subsec:cov_est}, arises from disregarding the drift in the stochastic process, and assuming that samples are drawn from a Gaussian distribution.
%
The third source comes from finite sampling effects, which is the subject of future research.
%
In this work, we address and discuss the first two sources of error (the finite sampling effects are the subject of future research).
%
We control the effects of the errors due to truncation of the Taylor expansion by adjusting $\sigma_{kernel}$; the higher-order terms in this expansion will be small for points which are close, such that adjusting $\sigma_{kernel}$ will allow us to only consider distances which are sufficiently accurate in our overall computation scheme.
%
Furthermore, we control the errors incurred by disregarding the drift by adjusting the time scale of our simulation bursts $\delta t$.
%
...
%
We present both analytical results for the error bounds, as well as an empirical methodology to set the parameters $\sigma_{kernel}$ and $\delta t$ for our method to accurately recover the slow variable(s).

\subsection{Error due to the observation function $\mathbf{f}$} \label{subsec:obsfn}
...
\end{equation}
%
Singer and Coifman \cite{singer2008non} showed that the error incurred by using the Mahalanobis distance to approximate the $L_2$-distance between points $\mathbf{z}(t)$ is $\mathcal{O} (\|\mathbf{y}_1 - \mathbf{y}_2 \|_2^4 )$ (see the Supplementary Materials for details).
%
...
%
We would like to note that this error does not depend on the dynamics of the underlying stochastic process (as we assume the covariances at each point on the manifold are known), but is only a function of the measurement function $\mathbf{f}$ and the distance between samples $\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|_2$.
%
...
%
We use simulation bursts to locally explore the dynamics on the manifold of observations in order to estimate the covariance at a point $\mathbf{y}(t)$ from data \cite{talmon2014manifold, talmon2014intrinsic}.
%
We write the elements of the estimated covariance matrix $\hat{\mathbf{C}}(\mathbf{y}(t), \delta t)$ as
...
\end{equation}
%
where $\delta t = o (\epsilon)$ is the length of the simulation burst.
...
%\end{equation}
%
By It\^{o}-Taylor expansion of $\mathbf{f}$ and $\mathbf{x}(t)$ \cite{kloeden1992numerical}, the entries in the error matrix are given by
%
...
\end{equation}
%
with $\epsilon=10^{-3}$, as a specific example of \eqref{eq:general_SDE}.
%
...
%
\caption{Data, simulated from \eqref{eq:specific_SDE} ($\epsilon = 10^{-3}$) for $3000$ time steps with $dt = 10^{-4}$. The data are colored by time.}
...
For the example in \eqref{eq:linear_transform}, the analytical covariance matrix is
...
\end{equation}
%
From \eqref{eq:cov_error}, we find the error matrix
%
\begin{equation}
...
%(provided $\frac{1}{\epsilon^2} \gg \sqrt{\delta t}$; this will be discussed further in Section~\ref{subsec:fastvar}).
%
These terms are shown in Figure~\ref{fig:cov_error}(a) as a function of $\delta t$.
%
We want to choose $\delta t = o(\epsilon)$, so that $\| \mathbf{E}_C \| \ll \| \mathbf{C} \|$ (the yellow shaded region in Figure~\ref{fig:cov_error} indicates where $\| \mathbf{E}_C \| < \| \mathbf{C} \|$), so that the errors in the estimated covariance are small with respect to the covariance.

When we do not analytically know the functions $\mathbf{f}$ or $\mathbf{g}$ or the value of $\epsilon$, we can find such a regime empirically by
...
\end{subfigure}
%
\caption{Errors in covariance estimation for linear example. (a) The analytical contributions to the covariance for the example in Section~\ref{subsec:linear_example} as a function of $\delta t$. (b) The average estimated covariance $\| \hat{ \mathbf{C}} \|$ as a function of $\delta t$. The average is computed over $10$ data points and using $50$ sample points to estimate each covariance. The shaded yellow region indicates the range of $\delta t$ over which the errors in the estimated covariance are less than the norm of the covariance. }
\label{fig:cov_error}
\end{figure}
...
\begin{figure}[t]
\centering
\epsfig{width=0.2\textwidth, file=data_linear_burst1.eps}
\epsfig{width=0.2\textwidth, file=data_linear_burst2.eps}
\epsfig{width=0.2\textwidth, file=data_linear_burst3.eps}

\epsfig{width=0.2\textwidth, file=data_linear_slow1.eps}
\epsfig{width=0.2\textwidth, file=data_linear_slow2.eps}
\epsfig{width=0.2\textwidth, file=data_linear_slow3.eps}

\epsfig{width=0.2\textwidth, file=data_linear_fast1.eps}
\epsfig{width=0.2\textwidth, file=data_linear_fast2.eps}
\epsfig{width=0.2\textwidth, file=data_linear_fast3.eps}

\epsfig{width=0.2\textwidth, file=data_linear_evals1.eps}
\epsfig{width=0.2\textwidth, file=data_linear_evals2.eps}
\epsfig{width=0.2\textwidth, file=data_linear_evals3.eps}
...
In the second example, our data from Section~\ref{subsec:linear_example} will be warped into ``half-moon" shapes via the function
...
%
This is a difficult class of problem in practice, as none of the observed
