...
partially supported by NSF (....) and the AFOSR (....).}}
...
%
Rather than being available in the form of an explicit analytical model,
such systems and their dynamics are often can only be observed as a data set which embodies dynamics on several time scales.
%
...
%
``Off-the-shelf" data mining methods are based on metrics (and thus, geometries) which are
not informed about the multiple-time-scale nature of the underlying system dynamics;
such methods cannot successfully recover the slow variables.
%
Here, we utilize both the local geometry and the {\em local} dynamics within the data set through a
metric which is both insensitive to the fast variables and more general than simple statistical averaging.
%
Through analysis of the method we provide conditions for successfully recovering the underlying slow variables,
as well as an empirical protocol guiding the selection the method parameters.
...
%
When the dynamically evolving variables are strongly coupled, resolving the
...
so that the reduction in complexity (in terms of number of variables) is counterpoised by an ``increase" in complexity
in terms of the nature of the reduced variable set model.
%
Here, we will study the special case of evolution equations which contain an inherent time scale separation:
systems where it is, in principle, possible to obtain a reduced system of differential equations
{\em without memory terms} in {\em only} the slow variables.
%
Such a formulation crucially hinges on knowing the ``true slow" variables within the system (the functions/combinations of ``primitive"
variables of the system) in terms of which the reduction is feasible.

Moving averages have often been used in simple cases to formulate lower-dimensional models.
%
%%%YGK  moving averages intrinsically contain memory - DISCUSS - is this a fair example or not ?
...
%
For well-studied systems, one often has some {\em a priori} knowledge of ``good observables" (such as phase field variables)
with which to formulate the reduced dynamics \cite{chen2002phase, wheeler1992phase}.
%
However, the appropriate observables may not be immediately obvious upon inspection for new complex systems.

In addition, given an explicit system of ordinary differential equations,
one can make numerical approximations, such as the quasi-steady state approximation or
the partial equilibrium approximation, to reduce the system dimensionality without introducing memory terms.
%
There has been some recent analytical work on extending and
generalizing such ideas to more complex systems of equations \cite{pavliotis2007parameter, ait2008closed, contou2011model, sotiropoulos2009model}.
...
%
In this paper, we will analyze multi(time)scale stochastic dynamical systems using data-driven methods.
%
Standard ``off-the-shelf'' manifold learning techniques which utilize the Euclidean distance will not be appropriate
for analyzing data from multiscale dynamical systems, since this metric does not account for the underlying dynamics
and their multiple time scales.
%
...
and can better recover the true underlying structure in the data by suppressing unimportant sources of variability \cite{rubner2000earth, simonyan2013fisher, xing2002distance, gepshtein2013image}.
%
...
the ``primitive system variable" evolution through a complex, nonlinear function.
%
Here, we will show the analogy between removing the effects of nonlinear observation functions (in the context of data analysis), and reducing the dynamical system to remove the effects of the fast variables.
%
Our approach will build a parameterization of the data which is consistent with the ``true" slow variables.
%
...
%
Even in such problems, we will show that our method has the potential to isolate the slow variables even when no variable is solely fast or slow. 
...
%
Our presentation and discussion is in terms of two-time-scale stochastic systems; yet we argue that
our framework and analysis extends to systems with multiple (more than one) time scale separations.
%
...
%
In the simple case of a linear drift function, i.e., when $a_i(\vec{x}(t)) = \mu _i x_i$ with $\mu_i < 0$, the probability density function of $x_i$ approaches a Gaussian with
(finite) variance $\mu_i$.
%
The time constant of the approach of the variance to equilibrium is $-1/\mu_i$ for $i=1,\ldots,m$ and $-\epsilon/\mu_i$ for $i=m+1,\ldots,n$ \cite{...}.
%%%YGK --------------
%%% missing citation ?
%
Thus, the last $n-m$ variables of \eqref{eq:general_SDE} are ``fast",
 and approach a local equilibrium measure rapidly, while the first $m$ variables are ``slow".
%
A short burst of simulation will yield a ``cloud of points" which is broadly distributed in the fast directions
but narrowly distributed in the slow ones.
%
Under reasonable
%%%YGK
%%  smoothness, boundedness ? both ? Khazminski ?
%%
conditions on $a_i(\vec{x})$, the same can be said for more general drift
functions, where $\mu_i$ are the eigenvalues of the Jacobian of $\mathbf{a}(\vec{x})$.
%
...
as we require the square of the diffusivity to be of the same order as the drift as $\epsilon \rightarrow 0$.
%%%YGK
%%%  ref. Khazminski, Gardiner, Friedlin?
%%%
%
...
%
In general, we do not have access to data $\vec{x}(t)$ from the original SDE system, but instead, we are given some {\em observations}
of the ``primitive variables" in the form $\vec{y}(t) = \mathbf{f} (\vec{x}(t))$.
%
...
%
Given data $\vec{y}(t_1),\ldots,\vec{y}(t_N)$ on $\mathcal{M}$ we would like to recover a parametrization of the data that is one-to-one with the
...
Gaussian $x$ distribution with covariance $C_x$, the Mahalanobis distance between the points is defined as \cite{mahalanobis1936generalized}
\begin{equation}
...
%
It is important to note that, in practice, we {\em never construct} $\vec{z}(t)$ {\em explicitly}.
%
...
\end{equation}
here, $\| \cdot \|$ denotes the appropriate norm (in our case, the Mahalanobis distance), and $\sigma_{kernel}$ is the {\em kernel scale}
...
%
$\phi_0$ is a constant trivial eigenvector; the next few eigenvectors constitute a
parameterization/embedding set of coordinates for the data (modulo higher harmonics which characterize
...
As previously mentioned, we do not have access to ``primitive" data $\vec{x}(t)$ from the underlying original SDE system;
instead, we only have {\em measurements/observations} $\vec{y}(t) = \mathbf{f} (\vec{x}(t))$.
%
...
nonlinearities in the observation function $\mathbf{f}$ and in the drift $\mathbf{a}(\vec{x})$.
%
...
\begin{equation} \label{eq:mahalanobis_distance}
 ...
 \end{equation}
where $C(\vec{y}(t))$ is the covariance of the observed stochastic process {\em at the point} $\vec{y}(t)$,
%
and $\dagger$ denotes the Moore-Penrose pseudoinverse (since $d$ may exceed $n$).

In the simple linear case where $\mathbf{f} (\vec{x}) = A \vec{x}$, with $A \in \mathbb{R}^{d \times n}$, the covariance of the observed stochastic process is given by $C=AC_xA^T$. Let $A=U \Lambda V^T$ be the singular value decomposition (SVD) of $A$, where $U \in \mathbb{R}^{d \times n}$, $\Lambda \in \mathbb{R}^{n \times n}$, and $V \in \mathbb{R}^{n \times n}$.
%
...
\end{aligned}
\end{equation}
Hence evaluating the Mahalanobis distance of the observations $\mathbf{f}(\vec{x})$ using \eqref{eq:mahalanobis_distance} allows us to estimate the Euclidean distances of the rescaled variables $\vec{z}$ (in which the fast coordinates are collapsed).
...
%
Given a set of values $\vec{y}(t_1), \dots, \vec{y}(t_q)$ drawn from the local
...
%
Alternatively, we can compute a time series by running a single simulation for a time $q \delta t$ starting from $\vec{y}(t_0)$, and then estimate the covariance
from  the increments $\Delta \vec{y}(t_i) = \vec{y}(t_i) -\vec{y}(t_{i-1})$.
%
...
To compute the Mahalanobis distance in \eqref{eq:mahalanobis_distance}, we require $C$, the covariance of the observed stochastic process $\vec{y}(t)$.
%
...
%
Note that in practice, we compute $\hat{C}$ by running many simulations of length $\delta t$ starting from $\vec{x}(t)$, and use the sample average to approximate the expected values in \eqref{eq:estimated_cov_expected_value}.
%
...
%
In contrast, the slow variable is recovered when using the Mahalanobis distance, as the coloring in Figure~\ref{fig:NIV_versus_DMAPS}(a) (where the data are colored by the first diffusion maps variable) is consistent with the coloring in Figure~\ref{fig:initial_data} (where the data are colored by time).
...
%
We want to choose $\delta t$ in a regime where $\| E_C \| \ll \| C\|$ (indicated by the yellow shaded region in the plot), so that the errors in the estimated covariance are small.
...
%
This provides an estimate of $\hat{C} = C + E_C$ as a function of $\delta t$.
%
...
%
Figure~\ref{fig:cov_error}(b) shows the empirical $\| \hat{C} \|$ as a function of $\delta t$ for the data in Figure~\ref{fig:initial_data}, and the knee in this curve is consistent with the crossover point in Figure~\ref{fig:cov_error}(a).
...
\end{subfigure}
%
\caption{Errors in covariance estimation for linear example. (a) The analytical contributions to the covariance for the example in Section~\ref{subsec:linear_example} as a function of $\delta t$. (b) The average estimated covariance $\| \hat{C} \|$ as a function of $\delta t$. The average is computed over $10$ test points and using $50$ sample points to estimate the covariance. The shaded yellow region indicates the range of $\delta t$ over which the errors in the estimated covariance are small. }
\label{fig:cov_error}
\end{figure}
...
%%%YGK
% this must be said better - I know what you mean, but a reader will not...
% we can discuss, but write this so that a reader can understand it.
%
...
%%%
%%% again, same item as above - say better
%
...
\caption{Relationship between changing $\delta t$ and recovery of the variables. From left to right, the columns correspond to $\delta t = 10^{-6}, 10^{-5}, 10^{-3}$.  (Row~1) Data (gray) and representative burst (red) used to estimate the local covariance. (Row~2) Correlation between first diffusion maps coordinate and the slow variable $x_1$. (Row~3) Correlation between the relevant diffusion maps coordinate and the fast variable $x_2$. Note that for $\delta t = 10^{-6}$ and $\delta t = 10^{-5}$, $x_2$ is correlated with $\phi_{10}$. When $\delta t = 10^{-3}$, $x_2$ is correlated with $\phi_6$. (Row 4) Diffusion maps eigenvalue spectra. The eigenvalues corresponding to the coordinates for the slow and fast modes are indicated by red circles. Note that when $\delta t$ is too large, the coordinate corresponding to the fast variable appears earlier in the spectrum. }
\label{fig:recover_fast}
\end{figure}

\subsection{Nonlinear measurement/observation function} \label{subsec:nonlinear_example}
...
%
It is important to note that this is the ``most difficult" type of problem in practice - none of the primitive
variables is ``purely fast" or ``purely slow", and the system appears, at first glance, to possess no separation
of time scales.
%
...
%
The Mahalanobis distance is an accurate approximation to the true intrinsic distance $\| \vec{z}(t_2) - \vec{z}(t_1) \|_2$ when $|E_M| \ll \| \vec{y}(t_2) - \vec{y}(t_1) \|^2_M$; this is indicated by the shaded yellow region in the plot.
%
...
\end{subfigure}
%
\caption{Errors in the Mahalanobis distance and the covariance estimation for the nonlinear example in \eqref{eq:nonlinear_function}. (a) The analytical expressions for the contributions to the distance approximation as a function of $\| \vec{y}_2 - \vec{y}_1\|_2$. (b) The average estimated Mahalanobis distance $\| \vec{y}_2 - \vec{y}_1\|^2_M$ as a function of the distance $\| \vec{y}_2 - \vec{y}_1\|_2$. The line $\| \vec{y}_2 - \vec{y}_1\|^2_M = \| \vec{y}_2 - \vec{y}_1\|_2$ is shown for reference. The yellow region indicates the range in which $\sigma_{kernel}$ should be chosen. (c) The analytical expressions for the contributions to the covariance as a function of $\delta t$. (d) The average estimated covariance $\| \hat{C} \|$ as a function of $\delta t$. The yellow region indicates the range of $\delta t$ over which the errors in the estimated covariance are small. }
...
%
$\|C \|$ and $\| E_C\|  $ are plotted as a function of $\delta t$ in Figure~\ref{fig:cov_error_nonlinear}(c); the shaded yellow portion denotes the region where $\| E_C \| \ll \| C \|$.
%
As in the previous example, we can empirically find where $\| E_C \| \ll \| C \|$  by plotting $\| \hat{C} \|$ as a function of $\delta t $ and looking for a knee in the plot.
%
...
%%%YGKdiscuss subsampling and fusion  and "redo local calculations" 

We have presented a methodology to compute a parameterization of a data set which ``inherently respects" the slow variables in the underlying dynamical system.
%
...
%%%YGK  it does not have to be a single time series
%%% need to say that in what we showed we used a single time series 
%% in general, we do not need a single time series - we need samples, and we need brief time series 
%% containing each sample
%% so, in a single time series we could contol teh rate of sampling
%% but in general we would have some sampling scheme and then "reinitize brief dynamics bursts" at every point of interest -
%% 

In the examples we presented, we controlled the time scale of sampling and could therefore set $\delta t$, the time scale over which to estimate the covariance, arbitrarily small (so that the drift does not appreciably change {\em and} we produce many sufficiently many sample points for an accurate estimate).
%
However, in many settings, such as a ``black-box'' simulator or previously collected historical data, it is not uncommon to have a fixed sampling rate.
%
%%%YGK == maybe this is where you say "if it is a code, we could go back and do short bursts, as we did in this paper -
%%% if not, it may be that we 
%
...
%%%
%%% YGK   help the reader, and me
%%% we told people "averages don't cut it" and now we tell them that moments of histograms "cut it"
%%% we need something more -----
%%%
%
Such intermediates reduce the effects of noise and permit a larger time step.
...
The methods presented here provide a bridge between traditional data mining and multi(time)scale dynamical systems.
%
With this interface established, one can now consider using such data-driven methodologies to ``extract" (explicitly, or in an equation-free framework \cite{....}) reduced models which, by respecting the (local) underlying dynamics, also respect the true ``slow geometry" of the data
%
Such reduced models hold the promise of accelerated analysis and redcued simulation of dynamical systems 
whose effective dynamics are obscure upon simple inspection.
