...
e_{C,22} =&
\frac{-1}{\epsilon^2} \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} ds_2 ds_1 \right] 
\end{aligned}
\end{equation}
...
e_{C,11}
=& 
2 f_{1,(2)}(\vec{x}_t) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} f_{1,(2,0)}(\vec{x}_{s_2}) ds_2 ds_1 \right] 
+ \mathbb{E} \left[  \int_t^{t+\delta t} \int_t^{s_2} f_{1,(0,2)}(\vec{x}_{s_1}) ds_1 ds_2\right] \right) \\
&+ \mathbb{E} \left[ \int_t^{t+\delta t}\left( \int_t^{s_2} f_{1,(2,2)}(\vec{x}_{s_1}) dW_{s_1, j}  \right) \left(  \int_t^{s_2} f_{1,(2,2)}(\vec{x}_{s_1}) dW_{s_1, j} \right) ds_2 \right] \\
&+ \mathcal{O} (\delta t^{5/2}) \\
=& 
\frac{4 x_2}{\sqrt{\epsilon}} \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} \frac{-4 x_2}{\epsilon^{3/2}} ds_2 ds_1 \right] 
+ \mathbb{E} \left[  \int_t^{t+\delta t} \int_t^{s_2} \frac{-2 x_2}{\epsilon^{3/2}} ds_1 ds_2\right] \right) \\
&+ \mathbb{E} \left[ \int_t^{t+\delta t}\left( \int_t^{s_2} \frac{2}{\epsilon} dW_{s_1, j}  \right)^2 ds_2 \right] 
+ \mathcal{O} (\delta t^{5/2}) \\
=& 
\frac{-8 x_2}{\epsilon^2} \left( 2 \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} x_2 ds_2 ds_1 \right] 
+ \mathbb{E} \left[  \int_t^{t+\delta t} \int_t^{s_2} x_2 ds_1 ds_2\right] \right) \\
&+ \frac{4}{\epsilon^2} \mathbb{E} \left[ \int_t^{t+\delta t}\left( \int_t^{s_2} dW_{s_1, j}  \right)^2 ds_2 \right] 
+ \mathcal{O} (\delta t^{5/2}) \\
%% STOPPED HERE
%
e_{C, 12} 
...
e_{C, 22} 
=&
 \sum_{j=1}^n f_{2,(j)}(\vec{x}_t) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} f_{2,(j,0)}(\vec{x}_{s_2}) ds_2 ds_1 \right] 
+ \mathbb{E} \left[  \int_t^{t+\delta t} \int_t^{s_2} f_{2,(0,j)}(\vec{x}_{s_1}) ds_1 ds_2\right] \right) \\
&+  \sum_{j=1}^n f_{2,(j)}(\vec{x}_t) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t + \delta t} f_{2,(j,0)}(\vec{x}_{s_2}) ds_2 ds_1 \right]
+ \mathbb{E} \left[ \int_t^{t+\delta t} \int_t^{s_2} f_{2,(0,j)}(\vec{x}_{s_1}) ds_1 ds_2 \right] \right) \\
&+ \sum_{j, k=1}^n \mathbb{E} \left[ \int_t^{t+\delta t}\left( \int_t^{s_2} f_{2,(j,k)}(\vec{x}_{s_1}) dW_{s_1, j}  \right) \left(  \int_t^{s_2} f_{2,(j,k)}(\vec{x}_{s_1}) dW_{s_1, j} \right) ds_2 \right] \\
&+ \mathcal{O} (\delta t^{5/2}) 
\end{aligned}
\end{equation}
...
\begin{itemize}
\item $f_{1,(1)} = 1$
\item $f_{1,(2)} = \frac{2 x_2}{\sqrt{\epsilon}} $
\item $f_{2,(1)} = 0 $
\item $f_{2,(2)} = \frac{1}{\sqrt{\epsilon}} $
\item $f_{1,(1,1)} = f_{1,(1,2)} = f_{1,(2,1)} = 0$
\item $f_{1,(2,2)} = \frac{2}{\sqrt{\epsilon}}$
\item $f_{2,(j, k)} = 0$
\item $f_{1,(1,0)}  = 0$
\item $f_{1, (2,0)} = \frac{-4}{\epsilon^{3/2}} x_2$
\item $f_{2,(1,0)} = f_{2,(2,0)}  = 0$
\item $f_{1, (0, 1)} = 0$
\item $f_{1,(0, 2)} = \frac{-2}{\epsilon^{3/2}} x_2$
\item $f_{2, (0, 1)} = f_{2, (0, 2)} = 0$
\end{itemize}
%
...
Using the relationship $\int_0^t W_s dW_s = \frac{1}{2}W_t^2 - \frac{1}{2} t$, we know that
\begin{equation}
\begin{aligned}
\int_t^{t+\delta t} dW_s &=& W_{\delta t} \\
\int_t^{t+\delta t} \int_t^{s_2} dW_{s_1} dW_{s_2} =
\int_t^{t+\delta t} W_{s_2} dW_{s_2} &=& \frac{1}{2} W_{\delta t}^2 - \frac{1}{2} \delta t  
\end{aligned}
\end{equation}
