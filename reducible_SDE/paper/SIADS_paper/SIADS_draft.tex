...
%
R.T. was supported by....
%
C.W.G. was supported by...
%
R.R.C. was supported by....
%
...
Multiple time scale stochastic dynamical systems are ubiquitous in science and engineering,
and the reduction of such systems and their models to only their slow components is often
essential for scientific computation and further analysis.
%
Rather than being available in the form of an explicit analytical model, often
such systems can only be observed as a data set which exhibits dynamics on several time scales.
%
...
%
Traditional data mining methods are based on metrics (and thus, geometries) which are
not informed of the multiscale nature of the underlying system dynamics;
such methods cannot successfully recover the slow variables.
%
Here, we present an approach which utilizes  both the local geometry and the {\em local dynamics} within the data set through a
metric which is both insensitive to the fast variables and more general than simple statistical averaging.
%
...
Following the methods of Mori \cite{mori1965transport}, Zwanzig \cite{zwanzig1961memory}, and others \cite{chorin2000optimal}, one can reduce the number of variables needed to describe a system of differential equations.
%
...
in this case, it is, in principle, possible to obtain a reduced system of differential equations in only the slow variables {\em without memory terms}.
%
...
Moving averages and subsampling have often been used in simple cases as appropriate functions of variables in which to formulate lower-dimensional models \cite{pavliotis2007parameter}.
%
%%%TODO  moving averages intrinsically contain memory - DISCUSS - is this a fair example or not ?
%%%  one cannot trivially initialize a simulation with "a given local average" 
%% we have addressed this with Gear (turned it into a solvable problem) BUT the question
%% is "does it create trouble to the reader ?"
%
However, if the underlying dynamics are sufficiently nonlinear, such statistics may
fail to capture the relevant structures and time scales within the data.
%
...
%
However, such observables may not be immediately obvious upon inspection for new complex systems.

In addition, given an explicit system of ordinary differential equations,
...
generalizing such ideas to more complex systems of equations \cite{ait2008closed, calderon2007fitting, contou2011model, givon2004extracting, pavliotis2007parameter,  sotiropoulos2009model}.
%%%TODO: add more SDE reduction references
%
...
usually through a distance metric \cite{Belkin2003, Coifman2006, coifman2005geometric, Roweis2000, Tenenbaum2000}.
%
...
%
%However, the Euclidean distance does not appropriately represent similarities,
%since simulation and experimental data often contain multiple sources of variability, only a few of which are important to describing the essence of the data.
%
Research efforts have addressed the construction of more informative distance metrics, which are less sensitive to noise
and can better recover the true underlying structure in the data by suppressing unimportant sources of variability \cite{gepshtein2013image, rubner2000earth, simonyan2013fisher, xing2002distance}.
%
The Mahalanobis distance is one such metric.
%
It was shown \cite{dsilva2013nonlinear, singer2008non, talmon2013empirical} that the Mahalanobis distance can remove the effect of {\em observing}
the underlying system variables through a complex, nonlinear function.
%
Here, we will show the analogy between removing the effects of such nonlinear observation functions (in the context of data analysis), and reducing the dynamical system to remove the effects of the fast variables.
%
...
%
Even when the underlying dynamical system is complex with nonlinear coupling between the fast and slow variables, we will show that our approach has the potential to isolate the underlying slow variables. 
...
%
Our presentation and discussion will address two-time-scale stochastic systems; however, we argue that
...
\end{subfigure}
%
\caption{(a) Schematic of a two-dimensional, two-time scale ordinary differential equation system where the value of $x_2$ becomes slaved to the value of $x_1$.
%
...
functions, where $\mu_i$ are the eigenvalues of the Jacobian of $\mathbf{a}(\mathbf{x})$ \cite{villani2009hypocoercivity}.
%
...
Assuming the sample average of $a_i(\mathbf{x})$ converges to a distribution which is only a function of the slow variables, by the averaging principle \cite{freidlin2012random}, we can write a reduced SDE in {\em only} the slow variables $x_1, \dots, x_m$.
%
...
In order to recover the slow variables from data, we will describe a local metric that collapses the fast directions.
%
...
%
Instead, we propose to use the Mahalanobis distance, which measures distances normalized by the respective variances in each principal direction.
%
...
\end{equation}
In particular, if $\mathbf{x}(t_1)$ and $\mathbf{x}(t_2)$ are samples of \eqref{eq:general_SDE}, then $\mathbf{C}_x = \mathrm{diag}(e_1, \ldots, e_n)$ where
...
\end{equation}
$\mathbf{z}(t)$ is therefore a stochastic process of the same dimension as $\mathbf{x}(t)$, rescaled so that each variable has unit diffusivity.
%
...
%
It was shown in \cite{singer2008non} that the Mahalanobis distance can be extended to approximate (to fourth order) the Euclidean distance between the rescaled samples $\mathbf{z}(t)$ from accessible $\mathbf{y}(t)$,
%
\begin{equation} \label{eq:mahalanobis}
\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|^2_M = \| \mathbf{z}(t_2) - \mathbf{z}(t_1) \|^2_2 + \mathcal{O}(\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|^4_2),
\end{equation}
and so we can recover a parameterization of the data which is consistent with the underlying system variables $\mathbf{x}(t)$, even when the data are obscured by a function $\mathbf{f}$.
%
...
%
Note that $\sigma_{kernel}$ induces a notion of locality: if $\|\mathbf{y}(t_i) - \mathbf{y}(t_j) \| \gg \sigma_{kernel}$, then $W_{ij}$ is negligible. Therefore, we only need our metric to be informative within a ball of radius $\sigma _{kernel}$.
%
...
\end{equation}
%
We compute the eigenvalues $\lambda_0, \dots, \lambda_{N-1}$ and eigenvectors $\phi_0, \dots, \phi_{N-1}$ of the matrix $\mathbf{A} = \mathbf{D}^{-1}\mathbf{W}$, and order them such that $|\lambda_0| \ge |\lambda_1| \ge \dots \ge |\lambda_{N-1}|$.
%
$\phi_0$ is a constant trivial eigenvector; the next few eigenvectors provide embedding coordinates for the data (modulo higher harmonics which characterize
...
\caption{Illustration of how to choose $\sigma_{kernel}$ and $\delta t$ appropriately.  The curvature effects and other nonlinearities should be negligible within a ball of radius $\sigma_{kernel}$ and within a time window $\delta t$.}
\label{fig:schematic}
\end{figure}
...
In the simple linear case where $\mathbf{f} (\mathbf{x}) = \mathbf{A} \mathbf{x}$, with $\mathbf{A} \in \mathbb{R}^{d \times n}$, the covariance of the observed stochastic process $\mathbf{f} (\mathbf{x})$ is given by $\mathbf{C}=\mathbf{AC}_x\mathbf{A}^T$. Let $\mathbf{A}= \mathbf{U} \mathbf{\Lambda} \mathbf{V}^T$ be the singular value decomposition (SVD) of $\mathbf{A}$, where $\mathbf{U} \in \mathbb{R}^{d \times n}$, $\mathbf{\Lambda} \in \mathbb{R}^{n \times n}$, and $\mathbf{V} \in \mathbb{R}^{n \times n}$.
%
...
\begin{equation}
\begin{aligned}
\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|^2_M &= (\mathbf{y}_2 - \mathbf{y}_1)^T  \mathbf{C}^{\dagger} (\mathbf{y}_2 - \mathbf{y}_1) \\
 &= (\mathbf{x}_2 - \mathbf{x}_1)^T \mathbf{A}^T \mathbf{C}^{\dagger} \mathbf{A} (\mathbf{x}_2 - \mathbf{x}_1) \\
 &= (\mathbf{x}_2 - \mathbf{x}_1)^T \mathbf{V \Lambda U}^T \mathbf{ U \Lambda}^{-1} \mathbf{V}^T \mathbf{C}_x^{-1} \mathbf{V \Lambda} ^{-1} \mathbf{U}^T \mathbf{U \Lambda V}^T (\mathbf{x}_2 - \mathbf{x}_1) \\
 &= (\mathbf{x}_2 - \mathbf{x}_1)^T \mathbf{C}_x^{-1}  (\mathbf{x}_2 - \mathbf{x}_1) \\
 &= \| \mathbf{x}_2 - \mathbf{x}_1 \|^2_M =  \| \mathbf{z}_2 - \mathbf{z}_1 \|^2_2 .
\end{aligned}
\end{equation}
Hence evaluating the Mahalanobis distance of the observations $\mathbf{f}(\mathbf{x})$ using \eqref{eq:mahalanobis_distance} allows us to estimate the Euclidean distances of the rescaled variables $\mathbf{z}$ (in which the fast coordinates are collapsed).
...
distance {\em in the rescaled variables} for general nonlinear observation functions $\mathbf{f}$.
...
%
One way to compute this set of points is to run $q$ simulations for a short time, $\delta t$, each starting from $\mathbf{y}(t_0)$.
%
Alternatively, we can compute a time series by running a single simulation for a time $q \delta t$ starting from $\mathbf{y}(t_0)$, and then estimate the covariance
...
%\begin{aligned}
%\| \mathbf{z}(t_2) - \mathbf{z}(t_1) \|^2_2 =&
%\frac{1}{2} \left(\mathbf{y}(t_2)-\mathbf{y}(t_1)\right)^T \left(C^\dagger (\mathbf{y}(t_1)) + C^\dagger (\mathbf{y}(t_2)) \right) (\mathbf{y}(t_2)-\mathbf{y}(t_1)) \\
...
%%
%Because we can empirically estimate $C(\mathbf{y}(t))$ from data (this will be discussed in Section~\ref{subsec:cov_est}), we choose to truncate the distance approximation in \eqref{eq:distance_taylor_expansion} at the second order term;
...
\left( g_{i, (j)} (\mathbf{y}(t_1)) g_{i, (k,l)} (\mathbf{y}(t_1)) -  g_{i, (j)} (\mathbf{y}(t_2)) g_{i, (k,l)} (\mathbf{y}(t_2)) \right) \\
& (\mathbf{y}_j(t_2) - \mathbf{y}_j(t_1))   (\mathbf{y}_k(t_2) - \mathbf{y}_k(t_1))(\mathbf{y}_l(t_2) - \mathbf{y}_l(t_1)) \\
+ \frac{1}{8} \sum_{i=1}^n \sum_{jklm=1}^d  &
\left( g_{i, (j,k)} (\mathbf{y}(t_1)) g_{i, (l,m)} (\mathbf{y}(t_1)) +  g_{i, (j,k)} (\mathbf{y}(t_2)) g_{i, (l,m)} (\mathbf{y}(t_2)) \right) \\
&(\mathbf{y}_j(t_2) - \mathbf{y}_j(t_1))  (\mathbf{y}_k(t_2) - \mathbf{y}_k(t_1))(\mathbf{y}_l(t_2) - \mathbf{y}_l(t_1)) (\mathbf{y}_m(t_2) - \mathbf{y}_m(t_1)) \\
+ \frac{1}{6} \sum_{i=1}^n \sum_{jklm=1}^d &
\left( g_{i, (j)} (\mathbf{y}(t_1)) g_{i, (k,l,m)} (\mathbf{y}(t_1)) +  g_{i, (j)} (\mathbf{y}(t_2)) g_{i, (k,l,m)} (\mathbf{y}(t_2)) \right) \\
& (\mathbf{y}_j(t_2) - \mathbf{y}_j(t_1))  (\mathbf{y}_k(t_2) - \mathbf{y}_k(t_1))(\mathbf{y}_l(t_2) - \mathbf{y}_l(t_1))(\mathbf{y}_m(t_2) - \mathbf{y}_m(t_1)) \\
+ & \mathcal{O} \left(\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|^6_2 \right) ,
\end{aligned}
\end{equation}
...
%\begin{equation} \label{eq:distance_taylor_expansion}
%\begin{aligned}
%&\| \mathbf{z}_2 - \mathbf{z}_1 \|^2_2 = \\
%& \frac{1}{2} \sum_{i=1}^n \sum_{jk=1}^d e_i \left( \left. \frac{\partial g_i}{\partial y_j} \right|_{\mathbf{y}_1} \left. \frac{\partial g_i}{\partial y_k} \right|_{\mathbf{y}_1} + \left. \frac{\partial g_i}{\partial y_j} \right|_{\mathbf{y}_2} \left. \frac{\partial g_i}{\partial y_k } \right|_{\mathbf{y}_2} \right) (y_{2,j} - y_{1,j}) (y_{2,k} - y_{1,k}) \\
%& + \frac{1}{2} \sum_{i=1}^n \sum_{jkl=1}^{d} e_i \left( \left. \frac{\partial g_i}{\partial y_j} \right|_{\mathbf{y}_1} \left. \frac{\partial^2 g_i}{\partial y_k \partial y_l} \right|_{\mathbf{y}_1} - \left. \frac{\partial g_i}{\partial y_j} \right|_{\mathbf{y}_2} \left. \frac{\partial^2 g_i}{\partial y_k \partial y_l} \right|_{\mathbf{y}_2} \right) (y_{2,j} - y_{1,j})  (y_{2,k} - y_{1,k})(y_{2,l} - y_{1,l}) \\
%& + \frac{1}{8} \sum_{i=1}^n \sum_{jklm=1}^d  e_i \left( \left. \frac{\partial^2 g_i}{\partial y_j \partial y_k} \right|_{\mathbf{y}_1} \left. \frac{\partial^2 g_i}{\partial y_l \partial y_m} \right|_{\mathbf{y}_1} + \left. \frac{\partial^2 g_i}{\partial y_j \partial y_k} \right|_{\mathbf{y}_2} \left. \frac{\partial^2 g_i}{\partial y_l \partial y_m} \right|_{\mathbf{y}_2} \right) (y_{2,j} - y_{1,j})  (y_{2,k} - y_{1,k})(y_{2,l} - y_{1,l})(y_{2,m} - y_{1,m}) \\
%& + \frac{1}{6} \sum_{i=1}^n \sum_{jklm=1}^d e_i \left( \left. \frac{\partial g_i}{\partial y_j} \right|_{\mathbf{y}_1} \left. \frac{\partial^3 g_i}{ \partial y_k \partial y_l \partial y_m} \right|_{\mathbf{y}_1} + \left. \frac{\partial g_i}{\partial y_j} \right|_{\mathbf{y}_2} \left. \frac{\partial^3 g_i}{ \partial y_k \partial y_l \partial y_m} \right|_{\mathbf{y}_2} \right) (y_{2,j} - y_{1,j})  (y_{2,k} - y_{1,k})(y_{2,l} - y_{1,l})(y_{2,m} - y_{1,m}) \\
%& + \mathcal{O} (\|\mathbf{y}_1 - \mathbf{y}_2 \|^6 ),
%\end{aligned}
%\end{equation}
%}
%
From this expression, the error incurred by using the Mahalanobis distance to approximate the $L_2$-distance between points $\mathbf{z}(t)$ is $\mathcal{O} (\|\mathbf{y}_1 - \mathbf{y}_2 \|^4 )$ and a function of the second- and higher-order derivatives of $\mathbf{g}$, as well as the distance between samples $\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|_2$.
%
...
%
From \eqref{eq:dmaps_kernel}, distances which are greater than $\sigma_{kernel}$ are negligible in the diffusion maps computation because of the exponential kernel.
%
Therefore, we want to choose $\sigma_{kernel}^2$ on the order of $\|\mathbf{y}(t_2) - \mathbf{y}(t_1)\|^2$ in a regime where $| E_M(\mathbf{y}(t_1), \mathbf{y}(t_2))|  \ll \|\mathbf{y}(t_2) - \mathbf{y}(t_1)\|^2$.
%
...
To compute the Mahalanobis distance in \eqref{eq:mahalanobis_distance}, we require $C$, the covariance of the observed stochastic process $\mathbf{y}(t)$.
%
We will use simulation bursts to estimate the covariance at a point $\mathbf{y}(t)$ from data.
%
We write the estimated covariance $\hat{\mathbf{C}}(\mathbf{y}(t), \delta t)$ as
...
%
We use simulation bursts to locally explore the dynamics on the manifold of observations in order to estimate the covariance.
...
\begin{equation}
E_C(\mathbf{y}(t), \delta t) = \hat{\mathbf{C}}(\mathbf{y}(t), \delta t) - \mathbf{C}(\mathbf{y}(t)).
\end{equation}
%
...
%\begin{aligned}
%\hat{C}_{ij} (\mathbf{x}_t, \delta t)
%=&
%\frac{1}{\delta t} \left( \mathbb{E} \left[ y_i (t+\delta t) y_j (t+ \delta t) \mid \mathbf{y}(t) \right]
%- \mathbb{E} \left[ y_i (t+\delta t) \mid \mathbf{y}(t) \right] \mathbb{E} \left[ y_j (t+\delta t) \mid \mathbf{y}(t) \right] \right)
%\\
%=& \sum_{k=1}^n \frac{1}{e_k} \left. \frac{\partial f_{i}}{\partial x_k} \right|_{\mathbf{x}(t)} \left. \frac{\partial f_{j}}{\partial x_k} \right|_{\mathbf{x}(t)}
%+ E_{C, ij} (\mathbf{x}(t), \delta t) + \mathcal{O}(\delta t^{3/2})
%=& \sum_{k=1}^n \frac{1}{e_k} \left. \frac{\partial f_{i}}{\partial x_k} \right|_{\mathbf{x}(t)} \left. \frac{\partial f_{j}}{\partial x_k} \right|_{\mathbf{x}(t)} \\
%&+ \frac{1}{\delta t} \sum_{k=1}^n f_{i,(k)}(\mathbf{x}(t)) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} f_{j,(k,0)}(\mathbf{x}(s_2)) ds_2 ds_1 \right]
%+ \mathbb{E} \left[  \int_t^{t+\delta t} \int_t^{s_2} f_{j,(0,k)}(\mathbf{x}(s_1)) ds_1 ds_2\right] \right) \\
%&+  \frac{1}{\delta t} \sum_{k=1}^n f_{j,(k)}(\mathbf{x}(t)) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t + \delta t} f_{i,(k,0)}(\mathbf{x}(s_2)) ds_2 ds_1 \right]
%+ \mathbb{E} \left[ \int_t^{t+\delta t} \int_t^{s_2} f_{i,(0,k)}(\mathbf{x}(s_1)) ds_1 ds_2 \right] \right) \\
...
\end{equation}
%
From \eqref{eq:cov_error}, the error in the covariance is $\mathcal{O}(\delta t)$ and a function of the derivatives of the observation function $\mathbf{f}$ and the drift $\mathbf{a}$.
%
We want to set $\delta t$ such that $\|E_C \| \ll \| \mathbf{C} \|$
...
using both the standard Euclidean disance and the Mahalanobis distance for the computation of the kernel in \eqref{eq:dmaps_kernel}.
%
...
\begin{subfigure}{0.4\textwidth}
\centering
\epsfig{height=2in, file=data_linear_DMAPS.eps}
\caption{}
\end{subfigure}
\begin{subfigure}{0.4\textwidth}
\centering
\epsfig{height=2in, file=data_linear_NIV.eps}
\caption{}
\end{subfigure}
%
\caption{Comparison of using the Euclidean distance and the Mahalanobis distance in multiscale data mining. (a) The data from Figure~\ref{fig:initial_data}, colored by the first diffusion map coordinate when using the Euclidean distance in the kernel. Note that we do {\em not} recover the slow variable. (b) The data from Figure~\ref{fig:initial_data}, colored by the first diffusion map coordinate when using the Mahalanobis distance in the kernel. The good correspondence between this coordinate and the slow variable is visually obvious.}
...
\end{equation}
%
Therefore, $\| \mathbf{C} \| = \mathcal{O} \left( \frac{1}{\epsilon} \right)$ and $\|E_C \| = \mathcal{O}\left(\frac{\delta t}{\epsilon^2} \right)$.
%
These terms are shown in Figure~\ref{fig:cov_error}(a) as a function of $\delta t$.
%
We want to choose $\delta t$ in a regime where $\| E_C \| \ll \| \mathbf{C} \|$ (indicated by the yellow shaded region in the plot), so that the errors in the estimated covariance are small.
...
%
This provides an estimate of $\hat{\mathbf{C}} = \mathbf{C} + E_C$ as a function of $\delta t$.
%
From Figure~\ref{fig:cov_error}(a), we can see that there should be a ``knee" in the plot of $\| \hat{\mathbf{C}} \|$ versus $\delta t$ when $\| E_C \|$ becomes larger than $\| \mathbf{C}\|$.
%
Figure~\ref{fig:cov_error}(b) shows the empirical $\| \hat{\mathbf{C}} \|$ as a function of $\delta t$ for the data in Figure~\ref{fig:initial_data}, and the knee in this curve is consistent with the crossover point in Figure~\ref{fig:cov_error}(a).

\begin{figure}[t]
\centering
\begin{subfigure}{0.4\textwidth}
\centering
\epsfig{height=2in, file=C_dt_analytical_linear.eps}
\caption{}
\end{subfigure}
%
\begin{subfigure}{0.4\textwidth}
\centering
\epsfig{height=2in, file=C_dt_linear.eps}
\caption{}
\end{subfigure}
%
\caption{Errors in covariance estimation for linear example. (a) The analytical contributions to the covariance for the example in Section~\ref{subsec:linear_example} as a function of $\delta t$. (b) The average estimated covariance $\| \hat{ \mathbf{C}} \|$ as a function of $\delta t$. The average is computed over $10$ test points and using $50$ sample points to estimate the covariance. The shaded yellow region indicates the range of $\delta t$ over which the errors in the estimated covariance are small. }
\label{fig:cov_error}
\end{figure}

\subsubsection{Recovery of the fast variable}

Note that, for the example in \eqref{eq:linear_transform}, $E_C$ is a constant diagonal matrix.
%
...
To see this behavior in our diffusion maps results, we first must discuss the interpretation of the diffusion maps eigenspectrum. 
%
The diffusion maps eigenvectors provide embedding coordinates for the data, and the corresponding elgenvalues provide a measure of the importance of each coordinate. 
%
...
%
These harmonics do not capture any new direction within the data set, but do appear as a separate eigenvectors/eigenvalues. 
%
...
%
As the time scale separation increases, the relative importance of the fast and slow directions will also increase.
%
This implies that the eigenvalue corresponding to the eigenvector which paremeterizes the slow direction will decrease, and the number of harmonics which appear between the slow and fast modes will increase. 
...
%
This means that the apparent time scale separation is smaller, the collapse of the fast variable is less pronounced relative to the slow variable, and the fast variable is recovered in a higher eigenvector.
%
...
%
It is important to note that this is the most difficult type of problem in practice, as none of the observed
variables are purely fast or slow, and the system appears, at first inspection, to possess no separation
of time scales.
%
...
%
The Mahalanobis distance is an accurate approximation to the true intrinsic distance $\| \mathbf{z}(t_2) - \mathbf{z}(t_1) \|_2$ when $|E_M| \ll \| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|^2_M$; this is indicated by the shaded yellow region in the plot.
%
...
%
$\|\mathbf{C} \|$ and $\| E_C\|  $ are plotted as a function of $\delta t$ in Figure~\ref{fig:cov_error_nonlinear}(c); the shaded yellow portion denotes the region where $\| E_C \| \ll \| \mathbf{C} \|$.
%
As in the previous example, we can empirically find where $\| E_C \| \ll \| \mathbf{C} \|$  by plotting $\| \hat{\mathbf{C}} \|$ as a function of $\delta t $ and looking for a knee in the plot.
%
...
%
When $\delta t$ corresponds to a region where $\|E_C \| \ll \| \mathbf{C} \|$, the slow variable is recovered by the first diffusion maps coordinate.
%
However, when $\delta t$ corresponds to a region where $\|E_C \| \gg \| \mathbf{C} \|$, the slow variable is no longer recovered.
...
%
Therefore, the approach can be used for data fusion: data from the same system collected via different measurement functions can be combined and merged into a single coordinate system. 

In the examples presented, we collected data from a single trajectory of a dynamical system, and then estimated the local covariance at each point in the trajectory using brief simulation bursts. 
%
However, the initial data need not be collected from a single trajectory, and other sampling schemes could be employed.
%
Brief time series are required to estimate the local covariances, but given a simulator, one can reinitialize brief simulation bursts which are sufficiently short and refined from each of the sample points.

In our examples, we controlled the time scale of sampling and could therefore set $\delta t$, the time scale over which to estimate the covariance, arbitrarily small.
%
However, in some settings, such as previously collected historical data, it is not uncommon to have a fixed sampling rate and be unable to reinitlize simulations.  
%
...
such as histograms, Fourier coefficients, or scattering transform coefficients \cite{mallat2012group}.
%
Such intermediates are more complex statistical functions than simple averages and can therefore capture more structure within the data.
%
...
%
With this interface established, one can now consider using such data-driven methodologies to extract reduced models (either explicitly, or implicitly via an equation-free framework \cite{kevrekidis2003equation, kevrekidis2004equation, erban2006gene, kevrekidis2009equation}) which also respect the underlying slow dynamics and geometry of the data.
%
