...
\begin{equation}
W_{ij} = \exp \left( -\frac{\|z_i - z_j \|_M^2}{\sigma_{kernel}^2} \right)
\end{equation}
...
%
From \eqref{eq:general_rescale}, we have
\begin{equation}
\begin{aligned}
...
%
For the general SDE formulation outlined in \eqref{eq:general_SDE}, we write the estimated covariance $\hat{C}$ \cite{...} as
...
\caption{The original data, simulated from \eqref{eq:specific_SDE} with $\epsilon = 10^{-3}$. We take $3000$ timesteps with $dt = 10^{-4}$}
...
To empirically find this regime, we can plot $\|C\|$ as a function of $\tau - t$. 


\begin{figure}[h]
\epsfig{width=0.5\textwidth, file=C_dt_linear.eps}
\epsfig{width=0.5\textwidth, file=data_linear_NIV_smalldt.eps}
\caption{The analytical bounds (blue curve) and empirical $\| C\|$ (red dots) as a function of the time burst $\tau - t$. }
\label{fig:cov_error}
\end{figure}
...
\epsfig{width=0.3\textwidth, file=data_nonlinear_NIV.eps}\epsfig{width=0.3\textwidth, file=data_nonlinear_DMAPS.eps}\caption{The data, simulated from \eqref{eq:specific_SDE}  with $\epsilon = 10^{-3}$. We take $3000$ timesteps with $dt = 10^{-4}$, after applying \eqref{eq:nonlinear_function}.}
