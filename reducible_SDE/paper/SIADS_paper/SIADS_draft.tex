...
%
Rather than having an explicit analytical model, such systems are often presented as a data set which exhibits dynamics on several time scales. 
%
We will focus on applying and adapting data mining and manifold learning techniques to detect the slow components in multiscale data.
%
Because typical off-the-shelf data-driven methods are based on metrics and geometry which are not informed about the underlying system dynamics, these methods cannot successfully recover the slow variables.
%
Here, we propose to utilize both the local geometry and local dynamics within the data set by constructing a metric which is both insensitive to the fast variables and more general than simple statistical averaging. 
%
...
%
It transforms a system of differential equations into a system of (lower-dimensional) {\em integro-}differential equations, and thus, does not yield a dramatic reduction in complexity. 
%
Here, we will the specific case of differential equations which contain an inherent time scale separation. 
%
...
When the dynamics are simple, statistics such as moving averages can describe the slow variables within the data.
%
...
In addition, given an explicit system of differential equations, one can make numerical approximations, such as the quasi-steady state approximation or the partial equilibrium approximation, to reduce the system dimensionality.  
%
...
%
The distances are then integrated into a global parameterization of the data through the solution of an eigenvector problem. 
%
...
\end{subfigure}
%
\caption{(a) Schematic of a two-scale two-dimensional dynamical system where the value of the second (fast) variable becomes slaved to the value of the first (slow) variable. In such an example, traditional data mining algorithms are sufficient to recover the slow variable. (b) Schematic of a two-scale two-dimensional dynamical system where the statistics of the second (fast) variable become slaved to the first (slow) variable. In such an example, traditional data mining algorithms will not recover the slow variable if the variance in the fast variable is too large. }
\end{figure}
...
%
Thus, the last $n-m$ variables of \eqref{eq:general_SDE} are fast and approach a local equilibrium rapidly, while the first $m$ variables are slow.
%
A short burst of simulation will yield a cloud of points which is widely distributed in the fast directions and narrowly distributed in the slow ones.
%
...
%
The ratio of the powers of $\epsilon$ in the drift and diffusion terms in \eqref{eq:general_SDE} is essential, as we require the square of the diffusivity to be of the same order as the drift as $\epsilon \rightarrow 0$. 
%
...
%
Instead, we propose to use the Mahalanobis distance which measures distances relative to the variances in each principal direction.
%
Using this metric, we still retain information about both the fast and slow directions and can capture more complex behavior within the data set.
...
%
The traditional Mahalanobis distance is defined for a fixed distribution, whereas we are dealing with a distribution that possibly changes due to nonlinearities in the observation function $\mathbf{f}$ and in the drift $\mathbf{a}(\vec{x})$. 
%
...
\begin{equation} \label{eq:mahalanobis_distance}
 ...
 \end{equation}
where $C(\vec{y}(t))$ is the covariance of the observed stochastic process at the point $\vec{y}(t)$, and $\dagger$ denotes the Moore-Penrose pseudoinverse (since $d$ may exceed $n$).
...
Following \cite{singer2008non}, we will show via Taylor expansion that the Mahalanobis distance between the observations \eqref{eq:mahalanobis_distance} approximates the Euclidean distance between the rescaled variables for more general nonlinear observation functions $\mathbf{f}$. 
...
%
Alternatively, we can compute a time series by running a single simulation for a time $q \delta t$ starting from $\vec{y}(t_0)$, and then estimate the covariance of the increments $\Delta \vec{y}(t_i) = \vec{y}(t_i) -\vec{y}(t_{i-1})$. 
%
...
%
We compute the diffusion maps embedding for the data in Figure~\ref{fig:initial_data}, using both the standard Euclidean disance and the Mahalanobis distance for the computation of the kernel in \eqref{eq:dmaps_kernel}.
%
...
\subsection{Nonlinear function} \label{subsec:nonlinear_example}
...
%
We need to use the Mahalanobis distance to obtain a parametrization that is consistent with the underlying dynamics.
...
%
Our method utilizes diffusion maps, a kernel-based manifold learning technique, with the Mahalanobis distance as the metric. 
%
...
In the examples we presented, we controlled the time scale of sampling and could therefore set $\delta t$, the time scale over which to estimate the covariance, arbitrarily small. 
%
...
%
For these cases, the data cannot be processed as given, and it is necessary to construct intermediate observers, such as histograms, Fourier coefficients, or scattering transform coefficients \cite{mallat2012group}.
%
Such intermediates reduce the effects of noise and permit a larger time step.

In addition, for this analysis, we have ignored the finite sampling effects in our estimation.
%
...
The methods presented here provide a bridge between traditional data mining and multiscale dynamical systems.
%
With this interface established, one can now consider using data-driven methodologies to write reduced models which not only respect the underlying geometry in the data, but also the underlying dynamics. 
%
Such reduced models would allow for accelerated analysis and simulation of new dynamical systems whose dynamics are unclear from simple inspection. 
