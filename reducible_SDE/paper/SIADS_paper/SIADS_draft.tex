...
%
In the simple case of a linear drift function, where $a_i(\vec{x}(t)) = \mu _i x_i$, the probability density function of $x_i$ approaches a Gaussian. 
%
If $\mu_i$ is negative, then the variance of $x_i$ is bounded by $\mu_i$, and the time constant of the approach of the variance to equilibrium is $-1/\mu_i, i=1,\ldots,m$ and $-\epsilon/\mu_i, i=m+1,\ldots,n$. 
%
Thus the last $n-m$ variables of \eqref{eq:general_SDE} are fast and approach a local equilibrium rapidly, while the first $m$ are slow, so that a short burst of simulation will yield a cloud of points that are widely distributed in the fast directions and narrowly distributed in the slow ones.
%
Under reasonable conditions on $a_i(\vec{x})$, the same can be said for a general drift function, where the $\mu_i$ are the eigenvalues of the Jacobian of $a(\vec{x})$.
%
Therefore, \eqref{eq:general_SDE} defines an $n$-dimensional stochastic system with $m$ slow variables and $n-m$ fast variables.
%
If $a_i(\vec{x})$ are adequately well behaved, then by the averaging principal \cite{...}, we can write a reduced SDE in {\em only} the $m$ slow variables. 
%
...
random distribution with covariance $C_x$, the Mahalanobis distance between the points is defined as
\begin{equation}
	\| \vec{x}_1 - \vec{x}_2 \| _M = \sqrt{ (\vec{x}_1 - \vec{x}_2)^T C_x^{-1} (\vec{x}_1 - \vec{x}_2)  }
\end{equation}
In particular, if $\vec{x}_1$ and $\vec{x}_2$ are samples of \eqref{eq:general_SDE}, then $C_x = \mathrm{diag}(e_1, \ldots, e_n)$ where
\begin{equation}
\begin{aligned}
e_i =& 1, \: & 1 \le i \le m \\
...
\end{aligned}
\end{equation}
Therefore, the Mahalanobis distance between samples from the SDE presented in \eqref{eq:general_SDE} is
...
%
We will show how we can approximate the Mahalanobis distance from data $\vec{y}(t)$ in Section~\ref{sec:analysis}.
...
\caption{Illustration of how to choose $\delta t$ and $\sigma_{kernel}$ appropriately. The data shows the evolution of the ``fast'' variable at a fixed value of the ``slow'' variable.  We must choose both parameters so that the curvature effects and other nonlinearities are negligible. TODO: make curves different (intrinsic vs. observations) }
\label{fig:schematic}
\end{figure}

The traditional Mahalanobis distance is defined for a fixed distribution, whereas we are dealing with a distribution that possibly changes because of any non-linearities in the observation function $\mathbf{f}$ and in the drift $\mathbf{a}(\vec{x})$. Consequently we use the following modified definition for the Mahalanobis distance between two points
...
In the simple linear case where $\mathbf{f} (\vec{x}) = A \vec{x}$, the covariance of the accessible sample is given by $C=AC_xA^T$. Let $A=U \Lambda V^T$ be the singular value decomposition (SVD) of $A$, where TODO. By the SVD, the pseudoinverse of the covariance matrix is $C^{\dagger} = U \Lambda^{-1} V^T C_x^{-1} V \Lambda ^{-1} U^T$. Consequently, the Mahalanobis distance \eqref{eq:mahalanobis_distance} is reduced to
\begin{align} 
...
%
One source of error is truncating the Taylor expansion and approximating the function $\mathbf{f}$ locally as a linear function. 
%
...
We want to relate the distance in the rescaled space, $\|\vec{z}(t_2) - \vec{z}(t_1)\|_2$, to the estimated Mahalanobis distance between the observations $\vec{y}(t) = \mathbf{f}(\vec{x}(t))$.
%
...
\end{equation}
%
By Taylor expansion of $\mathbf{g}(y) = \mathbf{f}^{-1}(y)$ around $\vec{y}(t_1)$ and $\vec{y}(t_2)$ and averaging the two expansions,
%
...
%
We would like to note that this error does not depend on the dynamics of the underlying stochastic process (as we are assuming we are given the analytic covariances at each point on the manifold), but is only a function of the measurement function $\mathbf{f}$. 
%
...
%
We therefore explicitly use the dynamics to estimate the covariance, and use simulation bursts to locally explore the dynamics on the manifold of observations. 
...
%
We therefore incur additional error due to finite sampling; this sampling error is ignored for the purposes of this analysis, and quantifying this error is the subject of future research. 
...
\caption{(a) Data from Figure~\ref{fig:initial_data_nonlinear}, colored by the first diffusion maps variable $\phi_1$ using the Mahalanobis distance with $\delta t = 10^{-7}$ and $\sigma_{kernel}^2 = 10^{-2}$. Note that the parametrization is one-to-one with the slow variable.  (b) Data from Figure~\ref{fig:initial_data_nonlinear}, colored by $\phi_1$ using the Mahalanobis distance with $\delta t = 10^{-7}$ and $\sigma_{kernel}^2 = 10^{1}$. We do not recover the slow variable because $\sigma_{kernel}$ is too large. (c) Data from Figure~\ref{fig:initial_data_nonlinear}, colored by $\phi_1$ using the Mahalanobis distance with $\delta t = 10^{-3}$ and $\sigma_{kernel}^2 = 10^{-2}$. We do not recover the slow variable because $\delta t$ is too large.  }
