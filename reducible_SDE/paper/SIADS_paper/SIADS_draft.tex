...
%
\begin{equation}
\mathbb{E} \left| \frac{1}{T} \int_t^{t + T} a_i(x_{t,1}, \dots, x_{t,m}, x_{s,m+1}, \dots, x_{s,n}) ds - \overline{a} (x_{t,1}, \dots, x_{t,m}) \right| < \kappa_i (T), 
\end{equation}
%
where $x_{t,i}$ denote the value of the $i^{th}$ component of $\vec{x}$ at time $t$, such that $\kappa_i(T) \rightarrow 0 $ as $T \rightarrow \infty$, then the averaging principle \cite{...} states that we can write an effective SDE in {\em only} the slow variables $x_1, \dots, x_m$. 
...
%
We require $\mathbf{g} = \mathbf{f}^{-1}$ be well-defined, and both $\mathbf{f}$ and $\mathbf{g}$ to be continuously differentiable to fourth order. 
%
Our goal is to recover a parameterization of observations $\mathbf{f}(\vec{x})$ which is one-to-one with the slow variables $x_1, \dots, x_m$ using data-driven techniques. 

\section{Local Invariant Metrics}

To locally recover the slow variables, we will build a metric that is invariant to the fast variables. 
%
...
\end{equation}
%
Therefore, \eqref{eq:general_rescale} transforms \eqref{eq:general_SDE} into the form of \eqref{eq:NIV_formulation}.
%
...
\end{equation}
%
The Mahalaonbis distance induces two important features. 
%
...
%
By Taylor expansion of $\mathbf{g}(y)$ around $\vec{y}_1$ and $\vec{y}_2$, averaging the two expansions, and substituting the relationships in \eqref{eq:variable_rescaling}, we obtain 
%
{\small
...
%
Therefore, we want to choose $\sigma_{kernel}^2$ on the order of $\|\vec{y}_2 - \vec{y}_1\|^2$ in a regime where $e_M(\vec{y}_1, \vec{y}_2) \ll \|\vec{y}_2 - \vec{y}_1\|^2$. 
%
...
C_{ij}(\vec{x}_t) = 
\sum_{k=1}^n \frac{1}{e_k} \left. \frac{\partial f_{i}}{\partial x_k} \right|_{\vec{x}_t} \left. \frac{\partial f_{j}}{\partial x_k} \right|_{\vec{x}_t} 
\end{equation}
%
We will use simulation bursts to estimate the local covariance.
%
Let $\vec{x}_t$ be the point at which  we want to estimate the covariance.
%
We then write the estimated covariance $\hat{C}(\vec{x}_t, \delta t)$, as
\begin{equation}\label{eq:estimated_cov_expected_value}
\hat{C}(\vec{x}_t, \delta t)
=
\frac{1}{\delta t} \left( \mathbb{E} \left[ f_1(\vec{x}_{t + \delta t}) f_2(\vec{x}_{t + \delta t}) \mid \vec{x}_t \right] 
- \mathbb{E} \left[ f_1(\vec{x}_{t + \delta t}) \mid \vec{x}_t \right] \mathbb{E} \left[ f_2(\vec{x}_{t + \delta t}) \mid \vec{x}_t \right] \right)
\end{equation}
%
where $\delta t > 0$ is the length of the simulation burst.
%
By It\={o}-Taylor expansion of $\mathbf{f}$ and $\vec{x}$,
...
\\
=& \sum_{k=1}^n \frac{1}{e_k} \left. \frac{\partial f_{i}}{\partial x_k} \right|_{\vec{x}_t} \left. \frac{\partial f_{j}}{\partial x_k} \right|_{\vec{x}_t} \\
&+ \frac{1}{\delta t} \sum_{k=1}^n f_{i,(k)}(\vec{x}_t) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t+\delta t} f_{j,(k,0)}(\vec{x}_{s_2}) ds_2 ds_1 \right] 
+ \mathbb{E} \left[  \int_t^{t+\delta t} \int_t^{s_2} f_{j,(0,k)}(\vec{x}_{s_1}) ds_1 ds_2\right] \right) \\
&+  \frac{1}{\delta t} \sum_{k=1}^n f_{j,(k)}(\vec{x}_t) \left( \mathbb{E} \left[ \int_t^{t+\delta t} \int_{s_1}^{t + \delta t} f_{i,(k,0)}(\vec{x}_{s_2}) ds_2 ds_1 \right]
+ \mathbb{E} \left[ \int_t^{t+\delta t} \int_t^{s_2} f_{i,(0,k)}(\vec{x}_{s_1}) ds_1 ds_2 \right] \right) \\
&+  \frac{1}{\delta t} \sum_{k,l=1}^n \mathbb{E} \left[ \int_t^{t+\delta t}\left( \int_t^{s_2} f_{i,(k,l)}(\vec{x}_{s_1}) dW_{s_1, k}  \right) \left(  \int_t^{s_2} f_{j,(k,l)}(\vec{x}_{s_1}) dW_{s_1, k} \right) ds_2 \right] \\
...
%
In practice, we compute $\hat{C}$ using \eqref{eq:estimated_cov_expected_value} by running many simulations of length $\delta t$ starting from $\vec{x}_t$, and using the sample average to approximate the expected values. 
...
%
We want to choose $\sigma_{kernel}^2$ in a regime where $e_M(\vec{y}_1, \vec{y}_2) \ll \| \vec{y}_2 - \vec{y}_1 \|^2_M$, so that the distances we utilize in the diffusion maps calculation are accurate. 
%
