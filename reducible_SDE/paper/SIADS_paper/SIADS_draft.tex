...
%
It can be shown that the Mahalanobis distance, $\| \cdot \|_M$, locally approximates: 
...
\end{equation}
of the SDE in  \eqref{eq:general_SDE}. 
%
\eqref{eq:general_SDE} now becomes
\begin{equation} 
\begin{aligned}
...
\begin{equation}
\| \vec{z}_2 - \vec{z}_1 \|^2_2 = \sum_{i=1}^n | z_{2,i} - z_{1,i} |^2 = \sum_{i=1}^n e_i |x_{2,i} - x_{1,i} |^2
\end{equation}
%
...
\end{equation}
%
where $C(\vec{y})$ is the local covariance of observed stochastic process at $\vec{y}$, which we can empirically estimate from data (this will be discussed in a subsequent section), and $C^{\dagger}(\vec{y})$ denotes the rank-$d$ Moose-Penrose pseudoinverse (since, in general, $d \ge n$).
%
...
= & \sum_{i=1}^n \frac{1}{e_i} \left. \frac{\partial f_j}{\partial x_i} \right|_{\vec{x}_t} \left. \frac{\partial f_k}{\partial x_i} \right|_{\vec{x}_t} \\ 
& + \frac{1}{\delta t} \sum_{i=1}^n \frac{1}{e_i^{3/2}}\left. \frac{\partial f_j}{\partial x_i} \right|_{\vec{x}_t} \mathbb{E} \left[ \left( \int_t^\tau dW_s^i  \right) \left( \int_t^{t + \delta t} \int_t^{s_2} \frac{\partial^2 f_k}{\partial x_i^2} dW_{s_1}^i dW_{s_2}^i  \right) \right] \\
& + \frac{1}{\delta t} \sum_{i=1}^n \frac{1}{e_i^{3/2}}\left. \frac{\partial f_k}{\partial x_i} \right|_{\vec{x}_t}  \mathbb{E} \left[ \left( \int_t^{t + \delta t} dW_s^i \right)  \left( \int_t^{t + \delta t} \int_t^{s_2} \frac{\partial^2 f_j}{\partial x_i^2}  dW_{s_1}^i dW_{s_2}^i \right) \right] \\
&+ \mathcal{O} (\delta t) 
\end{aligned}
\end{equation}
%
...
\frac{1}{\delta t} \left(  \mathbb{E} \left[ f_j(\vec{x}_{t + \delta t}) f_k(\vec{x}_{t + \delta t}) \right] - \mathbb{E}[f_j(\vec{x}_{t + \delta t})]\mathbb{E}[f_k(\vec{x}_{t + \delta t})]   \right)\\
= & \sum_{i=1}^n \frac{1}{e_i}\left. \frac{\partial f_j}{\partial x_i} \right|_{\vec{x}_t} \left. \frac{\partial f_k}{\partial x_i} \right|_{\vec{x}_t}  \\
 & + \frac{1}{\delta t} \sum_{i=1}^n \frac{1}{e_i} \left. \frac{\partial f_j}{\partial x_i} \right|_{\vec{x}_t}  \mathbb{E} \left[ \left(\int_t^{t + \delta t} dW_s^i \right)  \left(\int_t^{t + \delta t} \int_{t}^{s_2} \left(  \sum_{l=1}^n \frac{1}{e_l} \frac{\partial a_l}{\partial x_i} \frac{\partial f_k}{\partial x_l} \right)  dW_{s_1}^i ds_2 \right) \right] \\ 
 & + \frac{1}{\delta t} \sum_{i=1}^n \frac{1}{e_i}  \left. \frac{\partial f_k}{\partial x_i} \right|_{\vec{x}_t}  \mathbb{E} \left[ \left(\int_t^{t + \delta t} dW_s^i \right)  \left(\int_t^{t + \delta t} \int_{t}^{s_2} \left(  \sum_{l=1}^n \frac{1}{e_l} \frac{\partial a_l}{\partial x_i} \frac{\partial f_j}{\partial x_l} \right) dW_{s_1}^i ds_2 \right) \right] \\
