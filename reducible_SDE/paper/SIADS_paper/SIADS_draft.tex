...
%
The Mahalanobis distance, $\| \cdot \|_M$, is equivalent to
\begin{equation} 
...
To relate this metric with the multiscale SDE formulation in Section~\ref{subsec:multiscale_SDE}, we first define the vector $\vec{e} \in \mathbb{R}^n$ with
\begin{equation}
\begin{aligned}
e_i =& 1, \: & 1 \le i \le m \\
...
\end{equation}
%
Rescaling the data as so that all variables have unit diffusivity means that fast variables are collapsed and become $\sqrt{\epsilon}$ small, as observed in the right hand side of \eqref{eq:norm_z}.
%

In general, we will not have access to data $\vec{x}(t)$ from the original SDE system, but instead, we will have $\vec{y}(t) = \mathbf{f} (\vec{x}(t))$, where $\mathbf{f}: \mathbb{R}^n \mapsto \mathbb{R}^d$, $n \le d$, is a deterministic (perhaps nonlinear) function whose image is an $n$-dimensional manifold in $\mathbb{R}^d$.
%
Assuming $\mathbf{g} = \mathbf{f}^{-1}$ is well-defined, and both $\mathbf{f}$ and $\mathbf{g}$ are continuously differentiable to fourth order, then the Mahalanobis distance is invariant (to fourth order) to the function $\mathbf{f}$ \cite{...},
%
...
%
Since we do not have direct access to $\vec{z}(t)$, we will use \eqref{eq:mahalanobis} to approximate \eqref{eq:norm_z}, and show how we can approximate the Mahalanobis distance from data in Section~\ref{sec:analysis}.
...
\end{equation}
%
where $C(\vec{y}(t))$ is the covariance of the observed stochastic process at the point $\vec{y}(t)$, $\dagger$ denotes the Moose-Penrose pseudoinverse (since $d$ may exceed $n$), and $E_M$ is a fourth-order error term.
%
