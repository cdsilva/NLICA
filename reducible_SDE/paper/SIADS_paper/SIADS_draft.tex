...
%% TODO: where to discuss Radek PNAS?
...
%
$\phi_0$ is a constant trivial eigenvector; the next few eigenvectors provide embedding coordinates for the data, so that $\phi_j(i)$, the $i^{th}$ entry of $\phi_j$, provides the $j^{th}$ embedding coordinate for $\mathbf{y}(t_i)$ (modulo higher harmonics which characterize
...
\caption{Illustration of how to choose $\delta t$ and $\sigma_{kernel}$ appropriately.  The curvature effects and other nonlinearities should be negligible within a time window $\delta t$ and within a ball of radius $\sigma_{kernel}$.}
\label{fig:schematic}
\end{figure}
...
%
This implies that the eigenvalue corresponding to the eigenvector which paremeterizes the fast direction will decrease, and the number of harmonics of the slow mode which appear before the fast mode will increase. 
...
In the examples presented, the initial data came from a single trajectory of a dynamical system, and then estimated the local covariance at each point in the trajectory using brief simulation bursts. 
%
