...
In the simple case of a linear drift function, where $a_i(\vec{x}(t)) = -\mu _i x_i$, with $\mu_i > 0$, the probability density function of $x_i$ approaches a Gaussian with variance $\mu_i$. 
%
...
%
Under reasonable conditions on $a_i(\vec{x})$, the same can be said for a general drift function, where the $\mu_i$ are the eigenvalues of the Jacobian of $\mathbf{a}(\vec{x})$.
%
...
%
The ratio of the powers of $\epsilon$ in the drift and diffusion terms in \eqref{eq:general_SDE} is essential, as we need the square of the diffusivity to be of the same order as the drift as $\epsilon \rightarrow 0$. 
%
...
If $a_i(\vec{x})$ are sufficiently well behaved, then by the averaging principal \cite{...}, we can write a reduced SDE in {\em only} the slow variables $x_1, \dots, x_m$. 
%
...
%
It was shown in TODO:citeAmit that the Mahalanobis distance can be extended to approximate (to fourth order) the Euclidean distance between the rescaled samples $\vec{z}(t)$ from accessible $\vec{y}(t)$,
%
