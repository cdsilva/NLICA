...
random distribution with covariance $C_x$, the Mahalanobis distance between the points is defined as \cite{...}
\begin{equation}
...
%
It was shown in TODO:citeAmit that the Mahalanobis distance can be extended to approximate (to fourth order) the Euclidean distance between the rescaled samples $\vec{z}(t)$ from accessible $\vec{y}(t)$ as
%
...
%
We therefore use simulation bursts to locally explore the dynamics on the manifold of observations in order to estimate the covariance on the manifold. 
...
%
Therefore,t he Euclidean distance is not informative about the fast {\em or} the slow variables, and we need to use the Mahalanobis distance to obtain a parametrization that is consistent with the underlying dynamics.
