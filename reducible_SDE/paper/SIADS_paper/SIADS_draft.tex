...
\end{equation}
In particular, if $\mathbf{x}(t_1)$ and $\mathbf{x}(t_2)$ are samples of \eqref{eq:general_SDE}, then $\mathbf{C}_x^{-1} = \mathrm{diag}(e_1, \ldots, e_n)$ where
...
%
Note that $\sigma_{kernel}$ induces a notion of locality: if $\|\mathbf{y}(t_i) - \mathbf{y}(t_j) \| \gg \sigma_{kernel}$, then $W_{ij}$ is negligible. Therefore, we only need our metric to be informative within a ball of radius $\sigma _{kernel}$, as the Mahalanobis distance is in \eqref{eq:mahalanobis}.
%
...
\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|^2_M &= (\mathbf{y}_2 - \mathbf{y}_1)^T  \mathbf{C}^{\dagger} (\mathbf{y}_2 - \mathbf{y}_1) \\
 &= (\mathbf{x}_2 - \mathbf{x}_1)^T \mathbf{A}^T \mathbf{C}_x^{-1} \mathbf{A} (\mathbf{x}_2 - \mathbf{x}_1) \\
...
%}
%
From this expression, the error incurred by using the Mahalanobis distance to approximate the $L_2$-distance between points $\mathbf{z}(t)$ is $\mathcal{O} (\|\mathbf{y}_1 - \mathbf{y}_2 \|_2^4 )$ and a function of the second- and higher-order derivatives of $\mathbf{g} = \mathbf{f}^{-1}$, as well as the distance between samples $\| \mathbf{y}(t_2) - \mathbf{y}(t_1) \|_2$.
%
...
On first inspection, it would appear that our analysis indicates that $\sigma_{kernel}$ should be chosen arbitrairly small.
%
However, to obtain a meaningful parameterization of the data set, there must be a nonnegligable number of data points within a ball of radius $\sigma_{kernel}$ around each sample. 
%
...
%
In reality, both the number of samples used to estimate the covariances, as well as the density of sampled points on the manifold, affect the recovered parameterization and provide additional constraints on $\delta t$ and $\sigma_{kernel}$.
%
