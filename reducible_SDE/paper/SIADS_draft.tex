...
%
In the first example, our data will form a strip where the fast and slow remain uncoupled,
\begin{equation}
...
\| \mathbf{f}(Y_1) - \mathbf{f}(Y_2) \|^2_M = \| Y_1 - Y_2 \|^2_2 + \mathcal{O}(\| \mathbf{f}(Y_1) - \mathbf{f}(Y_2) \|^4_2)
\end{equation}
where the components of $Y$ have uncoupled noise with unit variance,
i.e., the variables $y_1, \dots, y_n$ are governed by the following SDEs:
\begin{equation} \label{eq:NIV_formulation}
dy_i = b(y) dt + dW_i
\end{equation}
and $Y = \begin{bmatrix} y_1 & y_2& \cdots & y_n \end{bmatrix}^T$.
...
\begin{aligned}
x_i = y_i, \: &1 \le i \le m \\
x_i = \frac{y_i}{\sqrt{\epsilon}} \: &1 \le i \le m
\end{aligned}
\end{equation}
...
\begin{equation} 
\begin{aligned}
dy_i &= b_i(y_1, \dots, y_n) dt + dW_i, \: 1 \le i \le m \\
dy_i &= -\frac{b_i(y_1, \dots, y_n)}{\sqrt{\epsilon}} dt + dW_i , \: m+1 \le i \le n
\end{aligned}
\end{equation}
where $b_i(y_1, \dots, y_n) = a_i (y_1, \dots, y_m, \frac{y_{m+1}}{\sqrt{\epsilon}}, \dots, \frac{y_n}{\sqrt{\epsilon}})$.
%
From \eqref{eq:general_rescale}, we can see that if $x_i = \mathcal{O}(1)$, then $y_i = \mathcal{O}({\sqrt{\epsilon}})$ for $ m+1 \le i \le n$, and 
\begin{equation}
\| Y_2 - Y_1 \|^2_2 = \sum_{i=1}^m \| y_{i,2} - y_{i,1} \|^2 = \sum_{i=1}^m \|x_{i,2} - x_{i,1} \|^2 + \epsilon \sum_{i=m+1}^n \|x_{i,2} - x_{i,1}\|^2
\end{equation}
%
...
\begin{aligned}
dy_1 &= adt + dW_1\\
dy_2 &= -\frac{y}{\epsilon} dt +  dW_2
\end{aligned}
\end{equation}
Therefore, $y_2$ is a stochastic variable whose diffusion is unity, and 
\begin{equation}
