...
\begin{figure}
\includegraphics[width=0.5\textwidth]{strip_cnts1}
\includegraphics[width=0.5\textwidth]{strip_cnts2}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\textwidth]{strip_discrete1}
\includegraphics[width=0.5\textwidth]{strip_discrete2}
\includegraphics[width=0.5\textwidth]{strip_nonuniform1}
\includegraphics[width=0.5\textwidth]{strip_nonuniform2}
...
%
Automatically detecting the ``important'' components will allow us to deduce when the dimension of the underlying dynamical system changes. 
%
...
We will illustrate these points using a model problem which arises in studies of cellular chemotaxis \cite{othmer2000diffusion}, where 
...
%
For example, when looking at a protein, the ``observers'' may be the locations of the $\alpha$-carbon atoms. 
%
Observers can also factor out other sources of variation. 
%
For example, the bispectrum \cite{...} and the scattering transform \cite{...} are both observers which are invariant to rotations. 
%
...
%
For molecular simulation data, it is often appropriate to use the root-mean-squared displacement {\em after} aligning configurations with respect to rotations, as global rotations of the entire molecule are not meaningful.
%
...
%
Another option is the Earth Mover's Distance, a multi scale distance metric which is stable and robust to noise.
